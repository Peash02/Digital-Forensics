...
def recover_deleted_files():
        ...
        for filename in files:
            if filename.endswith('.deleted'):
                ...
                if is_text_file(original_path):
                    recovered_name = filename.replace('.deleted', '')
                    recovered_path = os.path.join(RECOVERY_DIR, recovered_name)
                    shutil.copy2(original_path, recovered_path)
                    file_hash = calculate_hash(recovered_path)
    ...
    with open(REPORT_FILE, mode='w', newline='') as report:
        ...
        writer.writerow(["Original File", "Recovered File", "Size (bytes)", "SHA256 Hash", "Recovery Time", "Status"])
        writer.writerows(recovery_records)     
    print("\n" + "-"*45)
    print("Recovery process Completed")
    print(f"Total files recovered: {recovered_count}")
    print(f"Recovery report saved as: {REPORT_FILE}")
